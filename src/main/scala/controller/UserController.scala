...
import neko.server.http.{HttpRequest, HttpResponse}
import neko.server.http.{HttpStatus, OK, BAD_REQUEST, CONFLICT, UNAUTHORIZED, INTERNAL_SERVER_ERROR}
import neko.json.{Json, JsValue, JsonDecoder, JsonEncoder}

import application.entity.Token
import application.usecase.{FetchUserIdByToken, CreateUser, EditUserInfo}
  ...
  def edit(request: HttpRequest): HttpResponse = {
    val result: Either[HttpResponse, HttpResponse] = for {
      ...
      _ <- editUserInfo.execute(EditUserInfo.Request(userId, newUserName)).left.map {
        ...
        case EditUserInfo.Error.Unknown(e) => {
          println(e)
          cc.responseBuilder.build(INTERNAL_SERVER_ERROR)
        }
      }
...
object UserController {
  ...
  val createUserRequestDecoder = summon[JsonDecoder[CreateUser.Request]]
